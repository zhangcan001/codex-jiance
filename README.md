...
Codex Usage Monitor is a Windows desktop application for local Codex usage monitoring. The current milestone keeps the DEV-002 CLI detection foundation, adds safe local Codex App Server lifecycle control, completes the initialize/initialized handshake, and validates the installed CLI against its locally generated stable App Server schema. Account and usage integration is intentionally not included yet.
...
The following remain intentionally out of scope for DEV-006:
