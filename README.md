...
Codex Usage Monitor is a Windows desktop application for local Codex usage monitoring. The current milestone keeps the DEV-002 CLI detection foundation, adds safe local Codex App Server lifecycle control, completes the initialize/initialized handshake, validates the installed CLI against its locally generated stable App Server schema, and adds read-only Codex account inspection.

## Current development stage

`DEV-007 — Codex Account Service`
...
Account access is deliberately read-only. The service does not start login, cancel login, log out,
accept API keys, persist credentials, refresh tokens, read `auth.json` or cookies, or request usage,
rate-limit, credit, cost, model, thread, or turn data.
...
Rate limits, token usage, credits, and cost are not implemented yet.
...
The following remain intentionally out of scope for DEV-007:

- Authentication flows, rate-limit, token-usage, credit, or cost data
...
`DEV-008 — Codex Rate Limit Service`
