...
## Codex 用量监控器 v0.1.0 — 发布候选版

### 新增

- Windows 桌面版总览，用于查看 Codex 桌面版的官方额度观测和本地活动。
- 被动 Token 活动历史，以及项目和模型汇总。
- 带有明确可信度标记的消耗速率和额度耗尽预测。
- API 等效成本为按已观测 Token 和对应 API 单价折算的估算值，不是 Plus 订阅实际扣费金额。
...
- 本版本将正常运行架构迁移为桌面直连（Desktop Direct），直接读取 Codex 桌面版本地数据；正文后续统一使用“桌面直连”。
- 迁移前版本中的 App Server、账户读取和 CLI 运行控制仅作为历史架构保留，不属于当前桌面直连运行路径。

### 发布说明

- 在没有真实 Windows 签名证书时，本版本为未签名候选版本。
- 不包含更新器、GitHub 发布上传或自动安装步骤。
