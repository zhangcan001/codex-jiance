...
export default function HistoryPage() {
  ...
  return <div className="page">
    <header className="page-header"><div><p className="page-kicker">Observed history</p><h1>History</h1><p className="page-subtitle">Official rate-limit points and derived observed token deltas.</p></div><div className="section-heading__actions">{(["today", "7d", "30d", "all"] as Range[]).map((value) => <button className="button button--secondary button--compact" key={value} type="button" onClick={() => setRange(value)} disabled={range === value}>{value === "today" ? "Today" : value === "7d" ? "7 Days" : value === "30d" ? "30 Days" : "All"}</button>)}</div></header>
    ...
    {history ? <>
      ...
      <section className="section-block"><div className="section-heading"><h2>Rate Limit Usage History</h2><StatusBadge variant="success">Official</StatusBadge></div><div className="history-chart-grid"><Chart title="5 Hour" values={rate5h} suffix="used percent" /><Chart title="Weekly" values={weekly} suffix="used percent" /></div></section>
      <section className="section-block"><div className="section-heading"><h2>Token Usage History</h2><StatusBadge variant="warning">Derived</StatusBadge></div><p className="codex-message">Observed deltas · baseline-only snapshots are excluded from this chart.</p><Chart title="Derived observed token deltas" values={tokens} suffix="tokens" /></section>
